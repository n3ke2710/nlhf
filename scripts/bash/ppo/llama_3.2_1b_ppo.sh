...
if grep -q "base_url: https://api.deepinfra.com/v1/openai" "scripts/configs/ppo/tldr/llama_3.2_1b_ppo.yaml" 2>/dev/null; then
  if [ -z "${OPENAI_API_KEY:-}" ]; then
    echo "[warn] OPENAI_API_KEY not set, but config uses remote arbiter."
  fi
fi

tune run --nproc_per_node "$NPROC" src/rlhf/recipes/ppo.py --config "scripts/configs/ppo/tldr/llama_3.2_1b_ppo.yaml" "$@"
