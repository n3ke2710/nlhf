"""
Heuristic arbiter - быстрое локальное сравнение на основе эвристик.
"""
...
class HeuristicArbiter(BaseArbiter):
    """
    Эвристический арбитр для TL;DR.
    
    Оценивает качество саммари по:
    - Коэффициент сжатия (краткость)
    - Пересечение с оригиналом (релевантность)
    - Читаемость (структура, длина предложений)
    """
    
    def __init__(self, margin: float = 2.0):
        """
        Args:
            margin: Минимальная разница в скорах для победы (иначе tie)
        """
        self.margin = margin
    ...
    def compare(
        ...
        """Сравнить два ответа эвристически."""
    ...
    def _score_summary(self, original: str, summary: str) -> float:
        ...
        # Ideal: 5-15x compression
        if compression < 3:
            compression_score = compression * 10  # Too long
        elif compression > 20:
            compression_score = 100 - (compression - 20) * 2  # Too short
        else:
            compression_score = min(compression * 5, 100)
        
        # 2. Content overlap (keywords)
    ...
    def _extract_keywords(self, text: str) -> list:
        """Извлечь ключевые слова (без стоп-слов)."""
        STOP_WORDS = {
    ...
    def _readability_score(self, text: str) -> float:
        ...
        if avg_len < 5:
            len_score = avg_len * 10  # Too short
        elif avg_len > 30:
            len_score = max(0, 100 - (avg_len - 30) * 5)  # Too long
        else:
